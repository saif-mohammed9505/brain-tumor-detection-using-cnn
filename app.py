...
def load_model_or_stop() -> ModelService:
    try:
        ...
        st.error(
            f"🚫 Model file not found. Put `brain_tumor.h5` in the `models/` folder "
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        threshold = st.slider(
            ...
            step=0.05,
            format="%.2f",
            help="A scan is reported as a tumor when the model's probability reaches this value. "
            "Lower it to catch more tumors at the cost of more false alarms.",
        )
        show_input = st.toggle(
            "👁️ Show what the model sees",
            value=False,
    ...
    with left:
        upload = st.file_uploader(
            "📤 MRI image", type=config.ALLOWED_TYPES, label_visibility="collapsed"
        )
        if upload is not None:
            try:
                image = preprocess.load_image(upload)
            except (UnidentifiedImageError, OSError):
                st.error("🚫 That file can't be read as an image. Try a PNG or JPG export of the slice.")
        if image is not None:
            with st.container(key="film"):
    ...
    with right:
        if image is None:
            ...
            ui.render_result(prediction, threshold)
            if show_input:
                st.image(prediction.model_input, caption=f"🖼️ Model input, {width} × {height} px, {mode}")
